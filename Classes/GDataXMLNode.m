...
+ (NSString *)localNameForName:(NSString *)name {
  if (name != nil) {
    ...
    if (range.location != NSNotFound && range.location < [name length]) {
...
{
	NSError *error;
	NSString *attribname=[NSString stringWithFormat:@"./@%@",attributename];
	
	NSArray *plist=[self nodesForXPath:attribname error:&error];
	if(plist && [plist count])
	{
		return [[plist objectAtIndex:0] stringValue];
	}
	return @"";
}
...
{
	return [[self GetAttributeStringValue:attributename] intValue];
}

-(BOOL)GetAttrbuteBoolValue:(NSString	 *)attributename
{
	NSString *strvalue=[self GetAttributeStringValue:attributename];
	[strvalue uppercaseString];
	if([strvalue isEqualToString:@"YES"] || [strvalue isEqualToString:@"1"])
		return YES;
	return NO;
}
...
- (id)initWithXMLString:(NSString *)str options:(NSUInteger)mask error:(NSError **)error {
